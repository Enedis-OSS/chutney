...
class ChutneyServer(settings: ChutneyServerSettings) {
    ...
    companion object {
        ...
        private fun createCommandLine(settings: ChutneyServerSettings): GeneralCommandLine {
          ...
          val ideaServerJarFile = libFolder.walk().find { it.name.startsWith("chutney-server-") && it.name.endsWith("-boot.jar") }
                  ?: throw RuntimeException("chutney-server-<version>-boot.jar file not found in " + libFolder.absolutePath)
