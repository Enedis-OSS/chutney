...
final class MicrometerActionHelper {
    ...
    static Validator<String> durationStringValidation(String timeout, String label) {
        return of(timeout)
            .validate(s -> s == null || fr.enedis.chutney.action.spi.time.Duration.parseToMs(s) >= Long.MIN_VALUE , noException -> true, label + " is not parsable");
    }
    ...
    static Duration parseDuration(String duration) {
        return Duration.of(fr.enedis.chutney.action.spi.time.Duration.parse(duration).toMilliseconds(), ChronoUnit.MILLIS);
    }
