...
public class ScenarioIndexRepository implements IndexRepository<ScenarioEntity> {
    ...
    public void save(ScenarioEntity scenario) {
        ...
        luceneIndexRepository.update(byIdQuery(scenario.getId()), document);
    }
    ...
    private Query byIdQuery(Long id) {
        Query whatQuery = new TermQuery(new Term(WHAT, WHAT_VALUE));
        Query idQuery = new TermQuery(new Term(ID, id.toString()));
    ...
    private Hit highlight(Document doc, Query query) {
        QueryScorer scorer = new QueryScorer(query);
        Formatter formatter = new SimpleHTMLFormatter("<mark>", "</mark>");
        Highlighter highlighter = new Highlighter(formatter, scorer);
        String highlightedId = luceneIndexRepository.highlight(highlighter, ID, doc.get(ID), false);
        String highlightedTitle = luceneIndexRepository.highlight(highlighter, TITLE, doc.get(TITLE), false);
        String highlightedDescription = luceneIndexRepository.highlight(highlighter, DESCRIPTION, doc.get(DESCRIPTION), true);
        String highlightedContent = luceneIndexRepository.highlight(highlighter, CONTENT, doc.get(CONTENT), true);
        List<String> highlightedTags = TagListMapper.tagsStringToList(doc.get(TAGS)).stream()
            .map(tag -> luceneIndexRepository.highlight(highlighter, TAGS, tag, true))
