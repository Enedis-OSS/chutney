...
export class CampaignExecutionsHistoryComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.onExecuteSubscription = this.eventManagerService.subscribe('execute').pipe(
            switchMap(() => this.refreshCampaign())
        ).subscribe();
        this.onErrorSubscription = this.eventManagerService.subscribe('error').pipe(
            switchMap((event) => of(this.onMenuError(event)))
        ).subscribe();  
        this.onReplaySubscription = this.eventManagerService.subscribe('replay').pipe(
            switchMap(() => this.refreshCampaign())
        ).subscribe(); 
        this.campaignExecutionLast = this.eventManagerService.subscribe('executeLast').pipe(
            switchMap(() => this.replay())
        ).subscribe();  
    }
